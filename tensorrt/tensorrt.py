...
class LoadCalibrator(trt.IInt8EntropyCalibrator2):
    ...
    def get_batch(self, names):
        ...
        try:
            ...
            print("Calibration images pre-processed: {:}/{:}".format(
                self.batch_id, self.max_calib_size))
            self.batch_id += 1
    ...
    def write_calibration_cache(self, cache):
        ...
        with open(self.cache_file, "wb") as f:
            print("Caching calibration data for future use: {:}".format(
                self.cache_file))
            f.write(cache)
...
def get_int8_calibrator(calib_cache, calibration_loader, max_calib_size):
    ...
    if calib_cache and os.path.exists(calib_cache):
        print("==> Skipping calibration files, using calibration cache: {:}".
              format(calib_cache))
...
class TensorRTEngine:
    ...
    def __init__(
        ...
        assert precision in [
            ...
            "fp16",
            "int8",
        ], "precision must be fp32, fp16 or int8, but your precision is: {}".format(
            precision)
        ...
        if engine_file_path is not None and os.path.exists(engine_file_path):
            ...
            if use_int8 and builder.platform_has_fast_int8:
                ...
                config.int8_calibrator = get_int8_calibrator(
                    calibration_cache_file, calibration_loader,
            ...
            if shape_info is None:
                ...
                for i in range(len(onnx_model.graph.input)):
                    input_shape = [
                        x.dim_value for x in onnx_model.graph.input[0]
